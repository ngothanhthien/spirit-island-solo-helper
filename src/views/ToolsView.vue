...
<template>
  <div @click="save">
    <div ref="container" class="relative h-screen w-fit">
      <img
        alt=""
        class="h-full w-auto bottom-0 right-0 whitespace-nowrap"
        src="/img/spirits/vital_strength_of_the_earth_small.webp"
      />
      <div
        v-for="i in map.TOP"
        :style="cal(i - 1)"
        class="absolute bg-red-700 text-white rounded-full flex justify-center items-center"
      >
        {{ currentEnergy(i - 1) }}
      </div>
      <div
        v-for="i in map.BOT"
        :style="cal(i - 1, true)"
        class="absolute bg-blue-700 text-white rounded-full flex justify-center items-center"
      >
        {{ currentCardPlay(i - 1) }}
      </div>
    </div>
  ...
  <div>
    <div id="presence-tool" class="flex">
      ...
      <div>
        SPACE_Y: <input class="w-20" v-model="map.SPACE_Y" type="number" />
      </div>
    ...
    <div id="energy-tool">
      ...
      <div class="flex">
        <div class="bg-amber-300">
          ...
          <div>
            <input
              v-for="i in value.ENERGY"
              class="w-10"
              v-model="value.ENERGY[i]"
              type="text"
            />
          </div>
        </div>
        <div class="bg-blue-300">
          ...
          <div>
            <input
              v-for="i in value.CARD_PLAY"
              class="w-10"
              v-model="value.CARD_PLAY[i]"
              type="text"
            />
          </div>
        </div>
