...
const emit = defineEmits(['close', 'doVisionsOfAShiftingFuture'])
...
function doVisionOfAShiftingFutureEventDeck() {
  emit('doVisionsOfAShiftingFuture')
  emit('close')
}
</script>
<template>
  <div class="absolute bg-gray-900/30 w-full h-full top-0 left-0">
    <div
      class="flex flex-col h-[90%] w-full absolute z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-gray-900"
    >
      ...
      <div
        ...
        @click="doVisionOfAShiftingFutureEventDeck"
      >
