...
const pickMode = computed(() => props.type === 'power')
...
<template>
  <base-modal :is-height-fixed="false" :show-close-icon="false" @close="cardList.reset()"
    ...
    <template #body>
      <div class="flex flex-wrap gap-x-1 gap-y-4 py-2 px-2">
        <div v-for="card in cardList.getDeck" :key="card" class="flex flex-col items-center space-y-1"
          ...
          <game-card @click="cardZoom.setZoom(card, cardList.getDeck, 'discard')" class="rounded-xl cursor-zoom-in"
            :id="card" />
