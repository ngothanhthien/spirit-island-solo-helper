...
function closeModal() {
  ...
  if (Math.random() > 0.5) {
    ...
    ;[eventDeck.draw[lastIndex], eventDeck.draw[lastIndex - 1]] = [
      eventDeck.draw[lastIndex - 1],
      eventDeck.draw[lastIndex]
    ]
  }
}
</script>
...
<template>
  <div
    class="absolute w-full h-full top-0 left-0 bg-gray-900/30"
    @click.self="closeModal"
  >
    <div
      class="h-[90%] absolute z-50 top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2"
    >
