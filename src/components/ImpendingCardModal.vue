  ...
  >
    <div class="h-[90%] w-[90%] absolute z-50 top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 rounded-xl overflow-hidden bg-red-200">
      <div class="bg-gray-800 text-white px-3 py-2 relative">
        ...
        <span
          class="icon-x text-2xl absolute top-1 right-4"
          @click="$emit('close')"
        />
      </div>
      <div class="h-[80%]">
        ...
        <div class="h-[20%] flex items-center justify-center space-x-8">
          <base-button
            button-style="impending"
            :disabled="impendingCardStore.cards.length === 0"
            @click="impendingCardStore.allPlusOne()"
          >
            All +1 Energy
          </base-button>
          <base-button
            button-style="secondary"
            :disabled="!impendingCardStore.canAutoPlay"
            @click="autoPlayClick"
          >
            Auto play
          </base-button>
